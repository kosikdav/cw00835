...
$errorSleep = 30
$stdSleep = 1
#$ignoredOperations = @("CreateFlow","EditFlow")
        ...
        if ($resCount -ne 0) {
            ...
            if (-not ($indexError -or $queryError)) {
                ...
                foreach ($result in $results) {
                    $EnvironmentId = $EnvironmentName = $FlowId = [string]::Empty
                    write-host $result -ForegroundColor Cyan
                    $auditData = ConvertFrom-Json -InputObject $result.AuditData
                    if ($auditData.FlowDetailsUrl) {
                        $EnvironmentId = $auditData.FlowDetailsUrl.Replace("https://admin.powerplatform.microsoft.com/environments/", "")
                        $FlowId = $EnvironmentId.TrimEnd("/flowDetails")
                        $EnvironmentId = $EnvironmentId.Substring(0, $EnvironmentId.IndexOf("/"))
                        $FlowId = $FlowId.Substring($FlowId.LastIndexOf("/") + 1)
                        if ($PwrEnvironments_DB.ContainsKey($EnvironmentId)) {
                            $EnvironmentName = $PwrEnvironments_DB[$EnvironmentId].EnvironmentName
                        }
                    }
                    $auditObject = [pscustomobject]@{
                        ...
                        #UserUPN                 = $auditData.UserUPN;
                        UserType                = $auditData.UserType;
                        UserTypeInitiated       = $auditData.UserTypeInitiated;
                        RecordType              = $auditData.RecordType;
                        Workload                = $auditData.Workload;
                        ObjectId                = $auditData.ObjectId;
                        ClientIP                = $auditData.ClientIP;
                        FlowConnectorNames      = $auditData.FlowConnectorNames;
                        FlowDetailsUrl          = $auditData.FlowDetailsUrl;
                        EnvironmentId           = $EnvironmentId;
                        EnvironmentName         = $EnvironmentName;
                        FlowId                  = $FlowId;
                        #FlowName                = $FlowName;
                        LicenseDisplayName      = $auditData.LicenseDisplayName;
                        SharingPermission       = $auditData.SharingPermission;
