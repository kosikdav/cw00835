...
$LogFolder			= "exports"
$LogFilePrefix		= "aad-cap-audit"

$OutputFolder		= "aad-cap\audit"
$OutputFilePrefix	= "aad-cap-audit"
...
function Compare-PSObject {
    param(
        [Parameter(Mandatory=$true)]$OldObject,
        [Parameter(Mandatory=$true)]$NewObject,
        [string]$Path = ""
    )

    $changes = @()
    # Gather all properties from both objects
    $props = @($OldObject.PSObject.Properties.Name + $NewObject.PSObject.Properties.Name) | Sort-Object -Unique
    foreach ($prop in $props) {
        $oldVal = $OldObject.$prop
        $newVal = $NewObject.$prop
        $propPath = if ($Path) { "$Path.$prop" } else { $prop }

        if ($null -eq $oldVal -and $null -eq $newVal) {
            continue
        }
        elseif ($oldVal -is [pscustomobject] -and $newVal -is [pscustomobject]) {
            # Recurse into nested objects
            $changes += Compare-PSObject -OldObject $oldVal -NewObject $newVal -Path $propPath
        }
        elseif ($oldVal -is [System.Collections.IEnumerable] -and 
                -not ($oldVal -is [string]) -and
                $newVal -is [System.Collections.IEnumerable] -and 
                -not ($newVal -is [string])) {
            # Handle arrays – simple string compare for now
            if (($oldVal -join ",") -ne ($newVal -join ",")) {
                $changes += [pscustomobject]@{
                    Property = $propPath
                    OldValue = ($oldVal -join ",")
...
$UriFilter = "loggedByService+eq+'Conditional Access'+and+category+eq+'Policy'+and+activityDateTime+ge+$($start)+and+activityDateTime+le+$($strNow)"
...
Write-Log "Audit log events (Conditional Access) found: $($AuditLogEventsGrp.Count)"

$AuditLogEventsGrpCounter = 0
foreach ($AuditLogEvent in $AuditLogEventsGrp) {
    if ($AuditLogEvent.activityDisplayName -eq "Update conditional access policy"){
        write-host $AuditLogEvent.TargetResources.modifiedProperties.oldValue -ForegroundColor DarkYellow
        write-host $AuditLogEvent.TargetResources.modifiedProperties.newValue -ForegroundColor Yellow
        $oldValue = $AuditLogEvent.TargetResources.modifiedProperties.oldValue | ConvertFrom-Json
        $newValue = $AuditLogEvent.TargetResources.modifiedProperties.newValue | ConvertFrom-Json
        Compare-PSObject -OldObject $oldValue -NewObject $newValue
    }

    write-host $AuditLogEvent.AdditionalDetails -ForegroundColor Green
    write-host "----"
