...
$SrcMatchingAttribute = "extension_93b54ce056df45bd8f5f398753fa17c0_employeeNumber"
$DstMatchingAttribute = "extension_008a5d3f841f4052ac1283ff4782c560_employeeNumber"
...
#get source AAD users
Request-MSALToken -AppRegName $Src_AppReg_LOG_READER -TTL 30
$UriResource = "users"
$UriSelect = "id,userPrincipalName,displayName,mail,onpremisesSamAccountName,employeeId,onpremisesExtensionAttributes," + $SrcMatchingAttribute
...
[array]$SrcAADUsers = Get-GraphOutputREST -Uri $Uri -AccessToken $AuthDB[$Src_AppReg_LOG_READER].AccessToken -ProgressDots -Text "source AAD users"
write-host "SRC AAD users: $($SrcAADUsers.count)"
foreach ($user in $SrcAADUsers) {
	if ($user.userPrincipalName -like "karel.biza*") {
		write-host $user
	}
}
#get destination AAD users
Request-MSALToken -AppRegName $Dst_AppReg_LOG_READER -TTL 30
$UriResource = "users"
$UriSelect = "id,userPrincipalName,displayName,mail,onpremisesSamAccountName,employeeId,onpremisesExtensionAttributes," + $DstMatchingAttribute
$UriFilter = "userType eq 'Member' and onpremisesSyncEnabled eq true"
$Uri = New-GraphUri -Resource $UriResource -Version "v1.0" -Select $UriSelect -Filter $UriFilter -Top 999
[array]$DstAADUsers = Get-GraphOutputREST -Uri $Uri -AccessToken $AuthDB[$Dst_AppReg_LOG_READER].AccessToken -ProgressDots -Text "destination AAD users"
write-host "DST AAD users: $($DstAADUsers.count)"
#populate destination users DB hashtable with key = $DstMatchingAttribute and value = user object
foreach ($user in $DstAADUsers) {
	if ($user.$DstMatchingAttribute) {
		if ($user.userPrincipalName -like "karel.biza*") {
			write-host $user
		}
		...
		#$DstUsers_DB.Add($user.$DstMatchingAttribute, $DstUser)
	}
}
write-host "DST AAD users DB: $($DstUsers_DB.count)"
